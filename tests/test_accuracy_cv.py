...
def test_mcnnm_accuracy_no_covariates(tolerance=0.1):
    ...
    assert_close(jnp.mean(true_params['unit_fe']), jnp.mean(results.gamma), tolerance,
    ...
    assert_close(jnp.mean(true_params['time_fe']), jnp.mean(results.delta), tolerance,
                 "Estimated time fixed effects mean")

    print(f"\nTrue L mean: {jnp.mean(true_params['L']):.4f}")
    print(f"Estimated L mean: {jnp.mean(results.L):.4f}")
    assert_close(jnp.mean(true_params['L']), jnp.mean(results.L), tolerance,
                 "Estimated L mean")
...
def test_mcnnm_accuracy(tolerance=0.2):
    ...
    assert_close(jnp.mean(true_params['X_coef']), jnp.mean(results.H[:X.shape[1], :Z.shape[1]]), tolerance,
    ...
    assert_close(jnp.mean(true_params['Z_coef']), jnp.mean(results.H[:X.shape[1], :Z.shape[1]].T), tolerance,
    ...
    assert_close(jnp.mean(true_params['V_coef']), jnp.mean(results.beta), tolerance,
