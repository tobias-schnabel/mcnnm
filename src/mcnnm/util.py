...
def propose_lambda(proposed_lambda: Optional[Scalar] = None, n_lambdas: int = 6) -> Array:
    ...
    return jax.lax.cond(
        ...
        lambda _: generate_sequence(-3, 0),
        lambda _: generate_sequence(jnp.log10(proposed_lambda) - 2, jnp.log10(proposed_lambda) + 2),
        operand=None,
    )


def initialize_params(
    Y: jnp.ndarray, X: jnp.ndarray, Z: jnp.ndarray, V: jnp.ndarray
) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    """
    Initialize parameters for the MC-NNM model.

    Args:
        Y (jnp.ndarray): The observed outcome matrix of shape (N, T).
        X (jnp.ndarray): The unit-specific covariates matrix of shape (N, P).
        Z (jnp.ndarray): The time-specific covariates matrix of shape (T, Q).
        V (jnp.ndarray): The unit-time specific covariates tensor of shape (N, T, J).

    Returns:
        Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]: A tuple containing initial values for L,
    ...
    # Initialize beta with a consistent shape, using max(1, V.shape[2])
    beta_shape = jax.lax.max(V.shape[2], 1)
    beta = jnp.zeros(beta_shape)

    # If V.shape[2] is 0, we'll return an array of zeros with the same shape as beta
    beta = jax.lax.cond(
        V.shape[2] > 0,
        lambda x: x,
        lambda x: jnp.zeros_like(x),  # Return an array of zeros with the same shape
        beta,
    )
